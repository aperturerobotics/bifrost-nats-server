...
func (s *Server) solicitGateway(cfg *gatewayCfg, firstConnect bool) {
	...
	const connErrFmt = "Error connecting to %s gateway %q (attempt %v): %v"
	...
	for s.isRunning() {
		...
		*/
		...
		err := errors.New("TODO aperture-nats-server: dial gateway with name: " + cfg.Name)
		/*
			if err == nil {
		...
		*/
		if report {
			s.Errorf(connErrFmt, typeStr, cfg.Name, attempts, err)
		} else {
			s.Debugf(connErrFmt, typeStr, cfg.Name, attempts, err)
		}
		if isImplicit {
